...
exlcm::cmulti_command_t cmulti_command;
exlcm::cmulti_constants_t cmulti_constants;
exlcm::cmulti_crc_constants_t cmultit_crc_constants_t;
...
{
exlcm::cmulti_answer_t lcm_answer;
string answer;
...
// ------------------- Ende Kommandozeilenauswertung --------------------------------------
	...
	CMULTI cnet(device);
	cout << endl << "----- Here is the CMULTI-Server -----" << endl << endl; // prints Here is the CNET/Server
...
//		while( cnet_lcm.handle() == 0);
		while(1)
		{
			...
			if(cmulti_command.command.compare(0,9,"ServerIO?")==0) // dient zur internen Serverabfrage
			{
				lcm_answer.answer = "iO.";
				lcm_answer.error = cmulti_constants.answerTrue;
				cnet_lcm.publish(cmulti_command.target,&lcm_answer);
			...
			else
			{
				cnet.Open(convertBaudrate(baudRate),SerialPort::CHAR_SIZE_8,SerialPort::PARITY_NONE,SerialPort::STOP_BITS_1,SerialPort::FLOW_CONTROL_NONE);
				cnet.Flush();
				cnet.Send_Command(&cmulti_command);
				cout << "Command #" << cmulti_command.command << "# gesendet über RS485" << endl;
				if(cmulti_command.expect_answer)
				{
					cout << "Wait for Answer for " <<  (uint16_t)cmulti_command.timeout_ms << " ms" << endl;
					lcm_answer.error = cnet.Get_Answer(answer,cmulti_command.crcType,(uint16_t)cmulti_command.timeout_ms);
					lcm_answer.command_origin = cmulti_command.command;
					switch( lcm_answer.error ) //cnet.Get_Answer(answer,5)
					{
						case cmulti_constants.answerTrue:
						case cmulti_constants.answerFalse:
							lcm_answer.answer = answer;
						break;
						case cmulti_constants.answerWrong:
							lcm_answer.answer = "Server-Wrong-Answer!";
						break;
						case cmulti_constants.timeout:
							lcm_answer.answer = "Server-Timeout!";
						break;
						case cmulti_constants.crcError:
					...
					cout << "Answer ::" << lcm_answer.answer << ":: an !" << cmulti_command.target << "!  publiziert, Status: "  << endl;
				}
				cnet.Flush();
				cnet.Close();
